...
public class ChordPeer implements ChordApp
{
  ...
  public void run() throws IOException
  {
    ...
    Print("  commands: quit, add, remove, fail, inskey, status, lookup");
    Print("  debug: sha1, inrange, verbose, logex\n");
    ...
    do {
      ...
      if (tokens[0].equals("randkey"))
      {
        ...
        String randkey = randomString(16) + ".dat";
        n.addKey(randkey, randomData(KEY_TEST_SIZE * size));
      }
	...
	Print(n.toString());
      ...
      if (tokens[0].equals("inskey"))
      {
        if (tokens.length == 2) {
          try {
            n.addKey(getFilename(tokens[1]), getFileData(tokens[1]));
    ...
    try {
      for (i = 0; i < args.length; i++) 
      {
        if (args[i].equals("-connect")) {
          ...
          if (numkeys < 0) {
            System.out.println("warning: can't have less than 0 keys, using 0 keys");
            numkeys = 0;
          }
          i++;
